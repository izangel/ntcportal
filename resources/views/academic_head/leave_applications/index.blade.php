                            ...
                            @foreach($pendingApplications as $application)
                                <li class="bg-gray-50 p-4 rounded-lg shadow-sm flex items-center justify-between">
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-500 mt-1">Status: <span class="uppercase">{{ $application->ah_status }}</span></p>
                                    </div>
                                    <div>
                                        <a href="{{ URL::signedRoute('ah.leave_applications.review', ['leaveApplication' => $application->id]) }}" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-500 focus:bg-blue-500 active:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                            {{ __('Review') }}
                                        </a>
                                    </div>
                                </li>
