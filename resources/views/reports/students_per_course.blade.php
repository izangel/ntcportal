{{-- resources/views/reports/students_per_course.blade.php --}}

@extends('layouts.admin')

@section('header')
    <h2 class="font-semibold text-xl text-gray-800 leading-tight">
        {{ __('Students Per Course Report') }}
    </h2>
@endsection
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                <h3 class="text-lg font-medium text-gray-900 mb-4">Students Enrolled Per Course</h3>

                {{-- Filter Form --}}
                <div class="mb-6">
                    <form method="GET" action="{{ route('reports.studentsPerCourse') }}" class="flex flex-wrap items-center gap-4">
                        <div>
                        ...
                        @if ($selectedAcademicYearId || $selectedSemesterId)
                            <a href="{{ route('reports.studentsPerCourse') }}" class="text-sm text-gray-600 hover:text-gray-900">Clear Filters</a>
                        @endif
                    </form>
                </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">No. of Students</th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($reportData as $row)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $row->course_code }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $row->course_name }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">{{ $row->student_count }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
