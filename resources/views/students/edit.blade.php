    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                <h3 class="text-lg font-medium text-gray-900 mb-4">Edit Student: {{ $student->name }}</h3>
                ...
                <form method="POST" action="{{ route('students.update', $student) }}">
                    ...
                    <div class="mt-4">
                        ...
                        <x-input id="first_name" class="block mt-1 w-full" type="text" name="first_name" :value="old('first_name', $student->first_name)" required autofocus />
                        <x-input-error for="first_name" class="mt-2" />
                    </div>

                     <div class="mt-4">
                        <x-label for="last_name" value="{{ __('Last Name') }}" />
                        <x-input id="last_name" class="block mt-1 w-full" type="text" name="last_name" :value="old('last_name', $student->last_name)" required autofocus />
                        <x-input-error for="last_name" class="mt-2" />
                    </div>

                    <div class="mt-4">
                        <x-label for="student_email" value="{{ __('Student Email') }}" />
                        <x-input id="student_email" class="block mt-1 w-full" type="email" name="email" :value="old('email', $student->email)" required />
