        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($leaveApplications as $application)
                                <tr>
                                    ...
                                    {{-- In your table <td> --}}
                                    <td class="px-4 py-2">
                                        @if($application->hr_remarks && str_contains($application->hr_remarks, 'Recorded by HR'))
                                            ...
                                            <p class="text-[10px] text-gray-500 mt-1 italic">{{ $application->hr_remarks }}</p>
                                        @else
                                            <span class="text-xs text-gray-400">Self-filed</span>
                                        @endif
                                    </td>
                ...
                <div class="bg-white p-4 rounded-lg shadow mb-6 border border-gray-200">
                    <form method="GET" action="{{ route('leave_applications.export') }}" class="flex flex-wrap items-end gap-4">
                        <div>
                            <x-label for="start_date" value="From Date" class="text-xs" />
                            <input type="date" name="start_date" id="start_date" required
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 text-sm">
                        </div>
                        <div>
                            <x-label for="end_date" value="To Date" class="text-xs" />
                            <input type="date" name="end_date" id="end_date" required
                                class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-indigo-500 text-sm">
                        </div>
                        <div class="flex gap-2">
                            <button type="submit" 
                                    class="inline-flex items-center px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-xs font-bold rounded-md transition">
                            ...
                            {{-- Optional: Reset button to clear dates --}}
                            <a href="{{ route('hr.leave_applications.all') }}" class="px-4 py-2 text-xs text-gray-500 hover:text-gray-700 mt-2">Clear Filters</a>
                        </div>
                    </form>
                </div>
