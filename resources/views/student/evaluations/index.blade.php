    ...
    <div class="max-w-5xl mx-auto px-4">
        ...
        <div class="bg-white rounded-2xl border border-gray-200 p-6 mb-6 shadow-sm flex justify-between items-center">
            <div>
                ...
                <h1 class="text-xl font-bold text-indigo-500"> {{ $student->last_name}}, {{ $student->first_name}}
                <h1 class="text-xl font-bold text-gray-900">
                    Academic Evaluation:
                    {{ $semesterName !== 'N/A' ? $semesterName . ' Semester' : 'No Active Semester' }},
                    @if($activeSemester && $activeSemester->academicYear)
                        {{ $activeSemester->academicYear->start_year }}-{{ $activeSemester->academicYear->end_year }}
                    @else
                        Year Not Set
                    @endif
                </h1>
            ...
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-indigo-50/20 transition-colors">
                        <td class="px-6 py-5">
                            {{-- Accessing faculty name through the relationship --}}
                            <p class="text-sm font-bold text-gray-800">
                                {{ $block->faculty->first_name ?? 'N/A' }} {{ $block->faculty->last_name ?? '' }}
                            </p>
                        ...
                        <td class="px-6 py-5 text-center">
                            {{-- This 'has_evaluated' is the boolean we set in the Controller's map() function --}}
                        ...
                        <td class="px-6 py-5 text-right">
                            ...
                            @else
                                {{-- CRITICAL: Ensure this route passes $block->id (the CourseBlock ID) --}}
