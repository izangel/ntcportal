...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
        ...
        @else
            ...
            {{-- Pending Substitute Assignments Widget (For Teachers, Academic Head, HR & Admin) --}}
            @if(Auth::user()->employee && in_array(Auth::user()->employee->role, ['teacher', 'academic_head', 'hr', 'admin']) && isset($pendingSubstitutions) && count($pendingSubstitutions) > 0)
            <div class="bg-gradient-to-r from-orange-50 to-red-50 rounded-xl shadow-md border border-orange-200 p-6 mb-8">
                <div class="flex items-center justify-between mb-6">
                    <h4 class="text-lg font-bold text-orange-900 flex items-center">
                        <span class="p-2 bg-orange-100 rounded-lg mr-3">
                    ...
                    <span class="text-xs font-bold text-white bg-orange-500 px-3 py-1 rounded-full">
                        {{ count($pendingSubstitutions) }} Pending
                    </span>
                </div>

                <div class="space-y-3">
                    @foreach($pendingSubstitutions as $substitution)
                        <div class="bg-white rounded-lg border-l-4 border-orange-400 p-4 shadow-sm hover:shadow-md transition-all">
                            <div class="flex items-center justify-between">
                                <div class="flex-grow">
                                    <p class="text-sm font-bold text-gray-900">
                                        {{ $substitution->course_code }} - {{ $substitution->title }}
                                    </p>
                                    <p class="text-xs text-gray-600 mt-1">
                                        <strong>For:</strong> {{ $substitution->leaveApplication->employee->first_name }} {{ $substitution->leaveApplication->employee->last_name }}
                                    </p>
                                    <p class="text-xs text-gray-600">
                                        <strong>Leave Type:</strong> {{ $substitution->leaveApplication->leaveType->name }} 
                                        ({{ $substitution->leaveApplication->start_date->format('M d') }} – {{ $substitution->leaveApplication->end_date->format('M d') }})
                                    </p>
                                    <p class="text-xs text-gray-500 mt-2">
                                        <strong>Day/Time/Room:</strong> {{ $substitution->day_time_room ?? 'N/A' }}
                                    </p>
                                </div>
                                <div class="ml-4 flex gap-2">
                                    <form method="POST" action="{{ route('substitute.process_acknowledgement', $substitution->id) }}" class="inline-block">
                                        @csrf
                                        <input type="hidden" name="action" value="accept">
                                        <button type="submit" class="px-4 py-2 bg-green-500 hover:bg-green-600 text-white text-xs font-bold rounded-lg transition">
                                            Accept
                                        </button>
                                    </form>
                                    <form method="POST" action="{{ route('substitute.process_acknowledgement', $substitution->id) }}" class="inline-block">
                                        ...
                                        <button type="submit" class="px-4 py-2 bg-red-500 hover:bg-red-600 text-white text-xs font-bold rounded-lg transition">
                                            Reject
                                        </button>
                                    </form>
                                </div>
