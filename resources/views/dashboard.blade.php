                            ...
                            <div class="mt-6 flex gap-4">
                                @php
                                    if (Auth::user()->hasRole('admin')) {
                                        $route = 'admin.leave_applications.index';
                                    } elseif (Auth::user()->hasRole('hr')) {
                                        $route = 'hr.leave_applications.index';
                                    } elseif (Auth::user()->hasRole('academic_head')) {
                                        $route = 'ah.leave_applications.all';
                                    } else {
                                        $route = null;
                                    }
                                @endphp
        ...
        @else
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <div class="flex items-center justify-between mb-6">
                    ...
                    <a href="{{ route('important_dates.index') }}"
                        class="text-xs font-bold text-indigo-600 hover:underline uppercase">View Full Schedule</a>
                </div>
                    ...
                    @forelse($recentDates as $date)
                        @php
                            ...
                            $isOngoing = $today->between(
                                $date->start_date->startOfDay(),
                                ($date->end_date ?? $date->start_date)->endOfDay(),
                            );
                        @endphp
                        <div
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
                    ...
                    @empty
                        <div
                            class="col-span-full text-center py-8 bg-gray-50 rounded-xl border border-dashed border-gray-200 text-gray-400 italic">
                            No scheduled events found.</div>
                    @endforelse
                </div>
            </div>
                    ...
                    <div class="lg:col-span-2 space-y-8">
                        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                            <div class="flex justify-between items-center mb-4">
                                ...
                                <span
                                    class="text-[10px] font-black px-3 py-1 bg-indigo-50 text-indigo-600 rounded-full uppercase tracking-widest">
                            ...
                            <div class="divide-y divide-gray-100">
                                @forelse($upcomingSchedule as $block)
                                    <div
                                        class="py-4 flex justify-between items-center hover:bg-gray-50 transition px-2 rounded-lg">
                                        <div>
                                            <p class="font-bold text-gray-900">{{ $block->course->name }}</p>
                                            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-tight">
                                                {{ $block->course->code }} • {{ $block->faculty->first_name }}
                                                {{ $block->faculty->last_name }}
                                            </p>
                                        </div>
                                        <div class="text-right">
                                            <span
                                                class="text-sm font-bold text-indigo-600 block">{{ $block->schedule_string }}</span>
                                            <span
                                                class="text-[10px] text-gray-400 font-medium uppercase tracking-widest">{{ $block->room_name }}</span>
                                        </div>
                                    </div>
                                @empty
                                    <div class="text-center py-10">
                                        <p class="text-gray-400 italic text-sm">No courses found for the current semester.
                                        </p>
                                    </div>
                                @endforelse
                            </div>
            ...
            $stats = [
                ...
                @foreach($stats as $stat)
                    ...
                    {{-- Leave Management / Quick Actions (The other Half) --}}
                    <div
                        class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 flex flex-col justify-center text-center">
                        <h4 class="text-lg font-bold text-gray-800 mb-2">Leave Management</h4>
                        <p class="text-sm text-gray-500 mb-6">Review balances and submit applications.</p>
                        <a href="{{ route('leaveapplicationstatus') }}"
                            class="w-full py-3 bg-indigo-50 border border-indigo-100 rounded-xl text-indigo-700 text-sm font-bold hover:bg-indigo-100 transition">
                            Open Leave Portal
                        </a>
                    </div>
                ...
                @if (!Auth::user()->hasRole('student'))
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-8">
                        <div class="flex items-center justify-between mb-6">
                            ...
                            <span
                                class="text-xs font-bold text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full border border-indigo-100">
                                {{ now()->startOfWeek()->format('M d') }} —
                                {{ now()->startOfWeek()->addDays(4)->format('M d') }}
                            </span>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
                            @foreach ($daysOfWeek as $day)
                                ...
                                @endphp
                                <div
                                    class="flex flex-col min-h-[180px] rounded-xl border {{ $isToday ? 'bg-indigo-50/50 border-indigo-200 ring-2 ring-indigo-50' : 'bg-gray-50/30 border-gray-100' }}">

                                    <div
                                        class="p-3 text-center border-b {{ $isToday ? 'border-indigo-100 bg-indigo-100/30' : 'border-gray-100 bg-gray-50/50' }} rounded-t-xl">
                                        <p
                                            class="text-[10px] font-black uppercase tracking-tighter {{ $isToday ? 'text-indigo-600' : 'text-gray-400' }}">
                                            {{ $day->format('l') }}
                                        </p>
                                        <p
                                            class="text-xl font-black {{ $isToday ? 'text-indigo-700' : 'text-gray-800' }}">
                                            {{ $day->format('d') }}
                                        </p>
                                    </div>
                                    ...
                                    <div class="p-3 space-y-2 flex-grow">
                                        @forelse($dailyLeaves as $leave)
                                            <div
                                                class="bg-white p-2 rounded-lg border {{ $leave->approval_status === 'pending' ? 'border-amber-200' : 'border-gray-100' }} shadow-sm">
                                                <p class="text-[11px] font-bold text-gray-900 leading-tight">
                                                    {{ $leave->employee->last_name }}, {{ $leave->employee->first_name }}
                                                </p>
                                                <div class="flex items-center mt-1">
                                                    <span
                                                        class="w-1.5 h-1.5 rounded-full {{ $leave->approval_status === 'pending' ? 'bg-amber-400' : 'bg-green-500' }} mr-1.5"></span>
                                                    <span
                                                        class="text-[8px] font-black uppercase {{ $leave->approval_status === 'pending' ? 'text-amber-600' : 'text-green-600' }}">
                                        ...
                                        @empty
                                            <div class="h-full flex items-center justify-center py-8 opacity-20">
                                                <p
                                                    class="text-[10px] italic text-gray-400 font-bold uppercase tracking-widest">
                                                    No Leave</p>
                                            </div>
                                        @endforelse
                                    </div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
                    ...
                    @foreach ($stats as $stat)
                        <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-100 text-center">
                            <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{{ $stat['label'] }}
                            </p>
                            <p class="text-2xl font-black text-gray-900 mt-1">{{ $stat['val'] }}</p>
                            <div class="h-1 w-8 {{ $stat['color'] }} mx-auto mt-2 rounded-full"></div>
                        </div>
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                        <h4 class="text-lg font-bold text-gray-800 mb-4">Recently System Updates</h4>
                        <div class="space-y-3">
                            @forelse($recentUpdates as $update)
                                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg gap-3">
                                    <div class="flex items-center flex-1 min-w-0">
                                        <div
                                            class="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-700 font-bold text-xs mr-3 flex-shrink-0">
