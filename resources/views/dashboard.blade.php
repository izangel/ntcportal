...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-8">
        ...
        {{-- 1. Welcome & Notifications Header Section --}}
        <div class="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Welcome back, {{ Auth::user()->name }}!</h1>
                <p class="text-sm text-gray-500 mt-1">Here is what's happening in the portal today.</p>
            </div>
        ...
        {{-- Leave Quick Link Actions --}}
        @if(!Auth::user()->hasRole('student'))
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-8 flex flex-wrap gap-4 items-center">
                @php
                ...
                <div class="lg:col-span-2 space-y-8">
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                        ...
                        <div class="divide-y divide-gray-100">
                            ...
                            @empty
                                <p class="text-xs text-gray-400 text-center py-6 italic">No scheduled classes.</p>
                            @endforelse
                        </div>
                    </div>

                    {{-- Notifications Sub-card --}}
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                        <h4 class="text-lg font-bold text-gray-800 mb-4">🔔 Recent Notifications</h4>
                        <div class="space-y-3">
                            @forelse(Auth::user()->unreadNotifications->take(3) as $notification)
                                <div class="p-3 rounded-lg bg-gray-50 border border-gray-100">
                                    <p class="text-sm font-semibold">{{ $notification->data['title'] ?? 'Update' }}</p>
                                    <p class="text-xs text-gray-500">{{ $notification->data['message'] ?? '' }}</p>
                                </div>
                            @empty
                                <p class="text-xs text-gray-400 italic">No unread notifications.</p>
                            @endforelse
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-6">
                    <div class="bg-blue-600 rounded-xl shadow-lg p-6 text-white">
                        ...
                        <div class="mt-4 pt-4 border-t border-blue-400 flex justify-between text-xs">
                            <span>Active: <strong>{{ ($upcomingSchedule ?? collect())->count() }} Classes</strong></span>
        ...
        @else
                ...
                @if(isset($myCourses) && count($myCourses) > 0)
                    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                        <div class="p-4 border-b border-gray-50 bg-gray-50/50 flex justify-between items-center">
                            <h4 class="text-sm font-black text-gray-700 tracking-widest flex items-center uppercase">
            ...
            {{-- Full-Width Leave Summary Week Grid --}}
            @if(isset($daysOfWeek) && isset($leavesByDay))
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    ...
                    <div class="space-y-3">
                        ...
                        @empty
                            <div class="text-center py-4 bg-gray-50 rounded-lg">
                                <p class="text-[10px] text-gray-400 font-medium">No courses found.</p>
                            </div>
                        @endforelse
                    </div>
                </div>
